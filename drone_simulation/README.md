...
This package provides resources necessary for launching Gazebo simulation, along with `.launch` files for convenience.
...
This will start a new Gazebo instance (using `gazebo_ros` package), load a PX4 SITL instance, spawn a model and start ROS nodes. The PX4 console will be accessible in the terminal where `roslaunch` was performed.
...
A visual Gazebo plugin is used for the LED strip. An example of the plugin usage is provided in [`led_strip.xacro`](../drone/urdf/leds/led_strip.xacro).
